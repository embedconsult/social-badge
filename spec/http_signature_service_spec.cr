...
module SocialBadge
  private def signed_request(key : OpenSSL::PKey::RSA, key_id : String, host : String = "example.com")
...
TEXT
  ...
  describe HttpSignatureService do
    it "verifies a valid RSA-SHA256 signature" do
      ...
      request = signed_request(key, config.public_key_id)
    ...
    it "caches remote public keys within the TTL" do
      ...
      request = signed_request(key, key_id, "remote.example.com")
    ...
    it "refreshes a remote key after verification failure" do
      ...
      service = HttpSignatureService.new(config, key_fetcher: fetcher)
      request = signed_request(correct_key, key_id, "remote.example.com")
